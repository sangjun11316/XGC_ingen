...
mu0 = 4.0 * np.pi * 1.e-7;
nchease = 401;
chease_exec = '/home/ksk911211/CHEASE/src-f90/chease'
...
class eqdsk:
	...
	def read_eqdsk(self,filename):
		...
		print(f"nbbbs  {self.nbbbs}")
		print(f"limitr {self.limitr}")
	...
	def get_flux_contour(self):
		...
		R = np.linspace(Rmin,Rmax,self.nw)
		Z = np.linspace(Zmin,Zmax,self.nh)

		RR, ZZ = np.meshgrid(R,Z)
		
		psif = interp2d(self.R,self.Z,self.psirz)
		PSIRZ = psif(R,Z)
		self.contour=cntr.Cntr(RR,ZZ,PSIRZ)
	...
	def get_target_bnd(self):
		...
		self.rzbdyt = self.contour.trace(psint)[0]
	...
	def surface_zjz(self):
		...
		self.psinh = np.linspace(0,1.0,401)
...
#		chease_namelist1.append('                               CWIDTH=0.1, .10,.02,.05,\n');
	...
	def write_chease_namelist(self,filename='chease_namelist'):
	...
	def make_chease_input(self,filename1='EXPEQ',filename2='chease_namelist'):
		...
		self.write_chease_namelist(filename2)
		
		return

	def shape_postprocess(self):
...
#		txt.set_clip_on(False)
	...
	def draw_plot_comp(self,eq1,eq2,rhoplot=False):
		...
		ax2=plt.subplot2grid((2,4),(0,1))
		ax2.plot(xx1,eq1.pp*np.sign(eq1.pp[0])/1.e3,xx2,eq2.pp*np.sign(eq2.pp[0])/1.e3)
		ax2.set_title("P'($\psi$)")
		if rhoplot:	ax2.set_xlabel('Normalized radius ($\\rho_N$)')
		else:		ax2.set_xlabel('Normalized radius ($\psi_N$)')
		...
		#eq1.jplowf1 = np.copy(eq1.psin)
		#eq2.jplowf2 = np.copy(eq2.psin)

		#Rlow1 = interp1d(eq1.prhoR[:,0],eq1.prhoR[:,2],'slinear')
		#Rlow2 = interp1d(eq2.prhoR[:,0],eq2.prhoR[:,2],'slinear')

		#for i in range(len(eq1.psin)):  eq1.jplowf1[i] = - Rlow1(eq1.psin[i]) * eq1.pp[i]  - eq1.ffp[i]/mu0/Rlow1(eq1.psin[i])
		#for i in range(len(eq2.psin)):	eq2.jplowf2[i] = - Rlow2(eq2.psin[i]) * eq2.pp[i]  - eq2.ffp[i]/mu0/Rlow2(eq2.psin[i])
	...
	def write_files(self):
		...
		for i in range(len(self.psin)):
			dat[i,1] = self.jplowf[i]/1.e6
			dat[i,2] = self.jphighf[i]/1.e6
		
		self.write_file('Eqdsk_dat/currdens.dat',dat,'psin[a.u]\tLFS[MA/m2]\tHFS[MA/m2]')
